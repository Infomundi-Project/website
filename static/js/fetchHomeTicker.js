document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderTicker(data, containerId, title, iconClass) {
      ...
      data.forEach(entry => {
          ...
          if ( (containerId === 'stocks-ticker' || containerId === 'currencies-ticker') && entry.country && entry.country.code) {
              itemHtml += `
          ...
          if (containerId === 'stocks-ticker' && entry.country.name) {
              itemHtml += `<span style="color: white;">${entry.country.name}</span>`;
          } else if (containerId === 'currencies-ticker') {
              itemHtml += `<span style="color: white;">${entry.name_plural} (${entry.symbol})</span>`;
          } else {
              itemHtml += `<span style="color: white;">${entry.name}</span>`;
          }

          if (containerId === 'crypto-ticker' || containerId === 'currencies-ticker') {
              itemHtml += `
          ...
          if (containerId === 'currencies-ticker') {
              if (entry.name.slice(0, 3) === 'USD') {
                  itemHtml += `<span style="color: white;"> | 1 USD = ${entry.price} ${entry.symbol}</span>`;
              } else {
                  itemHtml += `<span style="color: white;"> | 1 ${entry.symbol} = ${entry.price} USD</span>`;
              }
          }
