...
let maximusTotalCharacters = loremIpsum.replace(/\s+/g, "").length; // Exclude spaces
...
function fetchAndRenderStorySummary(storyId) {
  ...
  const contentPlaceholder = document.querySelector(".maximus-summary-content-placeholder");
  const maximusApiResponse = document.querySelector(".maximus-summary-api-response");
