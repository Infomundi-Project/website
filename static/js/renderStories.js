document.addEventListener("DOMContentLoaded", function () {
  ...
  const modalTagsContainer = modalElement.querySelector(".modal-body .tags-container");
  ...
  // Grab it once
  const modalBookmarkIcon = document.getElementById("infomundiStoryModal").querySelector(".fa-bookmark");
      ...
      card.addEventListener("click", function (event) {
        ...
        if (
          ...
          if (storyData.publisher) {
            const { name, favicon_url } = storyData.publisher;
            ...
            if (favicon_url) {
              modalPublisherLogo.src = favicon_url;
          ...
          modalPublishedDate.innerHTML = `<i class="fa-regular fa-calendar me-2"></i>${timeAgo(storyData.pub_date)}`;
  ...
  function createStoryCard(item, index) {
    ...
    tipLogoImg.innerHTML = `
        <span data-bs-toggle="tooltip" data-bs-title="Source: ${item.publisher.name}">
          ${item.publisher.favicon_url ? `<img src="${item.publisher.favicon_url}" class="rounded" alt="${item.publisher.name} favicon image" width="30">` : `<i class="fa-solid fa-tower-cell"></i>`}
        </a>
    `;
