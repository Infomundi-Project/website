...
def has_external_links(text: str) -> bool:
    ...
    for pat in patterns:
        for match in re.finditer(pat, text, flags=re.IGNORECASE):
            findings.append({"pattern": pat, "match": match.group().strip()})
...
def sanitize_html(text: str) -> str:
    ...
    Example:
        ...
        'An evil() script'
    """
    return bleach.clean(text.strip(), tags=[], attributes=[], strip=True).strip()
...
def create_username_out_of_display_name(display_name: str) -> str:
    ...
    return sanitize_username(
        display_name.split(" ")[0][: config.MAX_USERNAME_LEN - 1]
